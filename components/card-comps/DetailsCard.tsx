...
export const DetailsCard = (props: NftCardProps) => {
    ...
    const handlePageId = () => {
        api.pagesForUser().then((resp) => {
            const pages = resp.data.items.sort((a, b) => parseISO(a.created_at).valueOf() - parseISO(b.created_at).valueOf()).reverse();
            setId(pages[2].id);
        });
    };
        ...
        None: () => <tr>
            <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                <div className="flex space-x-3">
                    <div className={"h-10 w-14 rounded-md border-1 " + s.BORDER} />
                </div>
            </td>
        </tr>,
        Some: (resp) => <tr >
